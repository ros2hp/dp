...
impl NodeType {
    ...
    // get_scalars returns scalars grouped by attribute partition
    ...
    pub fn get_11_edges(&self) -> Vec<(&str, &str)> {
        self.into_iter()
            .filter(|v| v.dt == ND && v.card == "1:1")
            .map(|v| (&v.c[..], &v.ty[..]))
            .collect()
    }
    ...
    pub fn get_attr_dt(&self, attr_sn: &str) -> &str {
        match self
            .into_iter()
            .filter(|&v| v.c == attr_sn)
            .map(|v| &v.dt)
            .last()
        {
            None => panic!(
                "get_attr_dt() not cached for attr_sn [{}] in type [{}]",
                attr_sn, self.long
            ),
            Some(s) => s,
        }
...
pub struct NodeTypes(pub Vec<Arc<NodeType>>);

impl<'a> NodeTypes {
    pub fn get(&self, ty_nm: &str) -> Arc<NodeType> {
    ...
    //             for (s, p) in ty.get_11_edges() {
    //                 h.insert(
    //                     s,
    //                     self.get(p) // returns clone of Arc<NodeType>
    //                         .get_scalars()
    //                         .into_iter()
    //                         .flat_map(|(_, v)| v)
    //                         .collect(),
...
pub async fn fetch_graph_types(
    ...
    //     items.into_iter().map(|v| v.into()).map(|v| Arc::new(v)).collect()
    // } else {
    //     vec![]
    // };
    if nt.len() == 0 {
    ...
    let mut ty_cache = NodeTypes_(nt);
    ...
    let mut x: Vec<Arc<NodeType>> = vec![];
    for i in ty_cache.0 {
        x.push(Arc::new(i));
    }
    let ty_cache_arc: NodeTypes = NodeTypes(x);
    println!("exit fetch_graph_types..");
    Ok((Arc::new(ty_cache_arc), graph_prefix_wdot))
}
