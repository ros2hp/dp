...
async fn main() -> Result<(), Box<dyn std::error::Error + Sync + Send + 'static>> {
    ...
    let child_edge = "Select puid,sortk,cuid from test_childedge order by puid,sortk"
        ...
        .map(&mut conn, |(puid, sortk, cuid): (Uuid, String, Uuid)| {
            ...
            match parent_edges.get_mut(&puid) {
                ...
                Some(e) => 
                    match e.get_mut(&types::SortK::new(&sortk)) {
                                None => {
    ...
    for puid in parent_node   {
        ...
        for &p_11_edge in p_11_edges { // Performance, . . 
                ...
                let Some(c_11_edges) = ty_with_11.get(c_type) else {panic!("{:?} not found in ty_with_11",c_type)};   // Performance 11 edges: Actor, Character, Film
                ...
                */
                let Some(v) = ty_with_11.get(node_types_.get(&p_11_edge.ty[..])) else {panic!("Expected Some got None for {}",p_11_edge.ty.as_str())};
                for &pg_11_edge in  v { // 1:1 attrs for Performance (Actor,Character,Film)
                        ...
                        match pg_11_edge.dt.as_str() { // edge predicate (uuidpred) 

                                "Nd" => {
                                    match di.nd {
                                        Some(v) => { 
                                            for b in v {
                                                items.entry(pg_sk_base.clone())
                                                    .and_modify( |v| v.nd.push(AttributeValue::B(Blob::new(b.as_bytes()))))
                                                    .or_insert(
                                                        DpPropagate {
                                        ...
                                        _ => {panic!("Data Error: Attribute {} in type {} has no value",pg_11_edge.c,c_type.long_nm())},
                                    }
                                },
                                _ => { panic!("Edge Query: expected Nd type...")}
                        }
                }
                let Some(c_attrs) =  ty_with_11.get(node_types_.get(&p_11_edge.ty[..])) else {panic!("type {} not found in ty_with_11",&p_11_edge.ty[..])};
                /*
                double propagate Scalar 
                */
                for &pg_11_edge in c_attrs { // 1:1 attrs for Performance (Actor,Character,Film)
                    ...
                    for (_,s) in &node_types_.get(&p_11_edge.ty[..]).get_scalars() {
                        ...
                        for &scalar in s {
                            ...
                            match pg_11_edge.dt.as_str() { // edge predicate (uuidpred) 
                                ...
                                "I"|"F" => {
                                    match di.ln {                                            
                                        Some(mut vv) => { 
                                            match vv.remove(0) {
                                                Some(s) => {
                                                    match items.get_mut(&pg_sk) {
                                                        None => {
                                                            items.insert(pg_sk.clone(), DpPropagate {
                                                                    nd: vec![],
                                                                    ls: vec![],
                                                ...
                                                _ => panic!("expected Some got None in cache for I or F type"),                                                 
                                                }
                                        },
                                        _ => {panic!("Data Error: Attribute {} in type {} has no value",pg_11_edge.c,c_type.long_nm())},
                                    }
                                ...
                                _ => {panic!("Unhandled match: got {} in match of dt",pg_11_edge.dt.as_str())},
                            }
                        }               
                    }
                }
